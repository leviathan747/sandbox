...
STRING_itoa( c_t A0xtumlsret[ESCHER_SYS_MAX_STRING_LEN], const i_t p_i )
{
  ...
  c_t * s=A0xtumlsret;i_t i;c_t * mapping[10]={0};
  /* ASSIGN mapping[9] = 9 */
 ...
 */
c_t *
STRING_getword( c_t A0xtumlsret[ESCHER_SYS_MAX_STRING_LEN], const i_t p_i, const i_t p_j, c_t * p_s )
{
  i_t len = strlen( p_s );
  c_t * result = A0xtumlsret;
  result[ 0 ] = 0;
  ...
  if ( !(p_i < 0 || p_i > len - 1) && !(lim < 0 || lim > len) ) {
    ...
    if ( (w_begin - p_s) < lim && (w_end - p_s) <= lim ) {
      ...
      c_t sub[ESCHER_SYS_MAX_STRING_LEN];
      STRING_substr( sub, (const i_t)(w_begin - p_s), (const i_t)(w_end - p_s), p_s );

      // Escher_strcpy
      c_t * dst = result;
      c_t * src = sub;
      if ( 0 != src ) {
        Escher_size_t i = strlen( src ) + 1;
        while ( ( i > 0 ) && ( *src != '\0' ) ) {
          --i;
          *dst++ = *src++;
        }
...
STRING_trim( c_t A0xtumlsret[ESCHER_SYS_MAX_STRING_LEN], c_t p_s[ESCHER_SYS_MAX_STRING_LEN] )
{
  ...
  if ( b < a ) {
    ...
    return STRING_substr( A0xtumlsret, (const i_t)(a - p_s), (const i_t)(b - p_s)+1, p_s );
  }
}
